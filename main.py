...
def extract_transaction_details(user_input: str, category_map: dict) -> dict:
    ...
    messages = [
        {
            ...
            "content": (
                ...
                "Instruction for category: Choose the single best fitting category from the Available categories list for the 'category' field. Pick the closest semantic match.\n"
    ...
    raw_output = generate(model, tokenizer, prompt=prompt, max_tokens=150, verbose=False)
...
def build_payload(parsed: dict, category_map: dict) -> dict:
...
def main():
    ...
    try:
        parsed = extract_transaction_details(raw_input, CATEGORY_MAP)
        print(f"\nParsed by LLM:\n{json.dumps(parsed, indent=2)}")
        payload = build_payload(parsed, CATEGORY_MAP)
